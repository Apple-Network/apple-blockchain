...
It also includes a verifiable delay function implementation that it imports from the [chiavdf repo](https://github.com/Apple-Network/applevdf) (in c/c++), and a proof of space implementation that it imports from the [chiapos repo](https://github.com/Apple-Network/chiapos). BLS signatures are imported from the [bls-signatures repo](https://github.com/Chia-Network/bls-signatures) as blspy. There is an additional dependency on the [chiabip158 repo](https://github.com/Apple-Network/applebip158). For major platforms, binary and source wheels are shipped to PyPI from each dependent repo. Then apple-blockchain can pip install those from PyPI or they can be prepackaged as is done for the Windows installer. On unsupported platforms, pip will fall back to the source distributions, to be compiled locally.
...
In the event an emergency fix is required for the release version of Apple, members of the Apple organization will create a feature branch from the current release branch `latest`.
...
sh install.sh -d
. ./activate
black . && isort benchmarks build_scripts apple tests tools *.py && mypy && flake8 benchmarks build_scripts apple tests tools *.py
...
The current official testnet is testnet10. Look at `apple/util/initial_config.yaml` to see the configuration parameters
