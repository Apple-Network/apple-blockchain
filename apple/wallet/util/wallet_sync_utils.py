...
from apple.wallet.util.peer_request_cache import PeerRequestCache
...
async def request_and_validate_removals(
    peer: WSAppleConnection, height: uint32, header_hash: bytes32, coin_name: bytes32, removals_root: bytes32
) -> bool:
...
async def request_header_blocks(
    peer: WSAppleConnection, start_height: uint32, end_height: uint32
...
async def _fetch_header_blocks_inner(
    all_peers: List[Tuple[WSAppleConnection, bool]],
...
async def fetch_coin_spend(height: uint32, coin: Coin, peer: WSAppleConnection) -> CoinSpend:
...
async def fetch_coin_spend_for_coin_state(coin_state: CoinState, peer: WSAppleConnection) -> CoinSpend:
