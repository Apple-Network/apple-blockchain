...
from apple.util.misc import VersionedBlob
from apple.wallet.cat_wallet.cat_info import CATCoinData, CRCATInfo
from apple.wallet.cat_wallet.cat_utils import CAT_MOD_HASH, CAT_MOD_HASH_HASH, construct_cat_puzzle
...
from apple.wallet.util.tx_config import TXConfig
...
from apple.wallet.vc_wallet.cr_cat_drivers import (
    CRCAT,
    CRCATMetadata,
    CRCATVersion,
    ProofsChecker,
...
class CRCATWallet(CATWallet):
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSAppleConnection, coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def _generate_unsigned_spendbundle(
            ...
            if first:
                ...
                if need_apple_transaction:
                    if fee > regular_apple_to_claim:
                        ...
                        apple_tx, apple_announcement = await self.create_tandem_apple_tx(
                            fee,
                            uint64(regular_apple_to_claim),
                            tx_config,
                        )
                        assert apple_announcement is not None
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
                            coin_announcements_to_assert={apple_announcement.name()},
                            conditions=extra_conditions,
                        )
                else:
                    innersol = self.standard_wallet.make_solution(
        ...
        return (
            SpendBundle(
                [
                    ...
                    *(
                        (
                            spend
                            for bundle in [apple_tx.spend_bundle]
            ...
            [*vc_txs, *([apple_tx] if apple_tx is not None else [])],
        )
    ...
    async def claim_pending_approval_balance(
        ...
        # Select the relevant APPLE coins
        if fee > 0:
        ...
        nonce: bytes32 = Program.to(
            [coin_as_list(c) for c in sorted(coins.union(apple_coins).union({vc.coin}), key=Coin.name)]
        ).get_tree_hash()
        ...
        if fee > 0:
            ...
            if apple_tx.spend_bundle is None:
                raise RuntimeError("Did not get spendbundle for fee transaction")  # pragma: no cover
            claim_bundle = SpendBundle.aggregate([claim_bundle, apple_tx.spend_bundle])
        else:
            apple_tx = None
        ...
        other_txs: List[TransactionRecord] = [
            ...
            *((dataclasses.replace(apple_tx, spend_bundle=None),) if apple_tx is not None else []),
        ]
