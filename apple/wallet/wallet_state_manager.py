...
from apple.consensus.coinbase import farmer_parent_id, pool_parent_id
from apple.consensus.constants import ConsensusConstants
from apple.data_layer.data_layer_wallet import DataLayerWallet
from apple.data_layer.dl_wallet_store import DataLayerStore
from apple.pools.pool_puzzles import (
...
from apple.rpc.rpc_server import StateChangedProtocol
...
from apple.util.db_synchronous import db_synchronous_on
from apple.util.db_wrapper import DBWrapper2
from apple.util.errors import Err
from apple.util.hash import std_hash
from apple.util.ints import uint16, uint32, uint64, uint128
from apple.util.lru_cache import LRUCache
from apple.util.misc import UInt32Range, UInt64Range, VersionedBlob
from apple.util.path import path_from_root
from apple.util.streamable import Streamable
from apple.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from apple.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, CRCATInfo
from apple.wallet.cat_wallet.cat_utils import CAT_MOD, CAT_MOD_HASH, construct_cat_puzzle, match_cat_puzzle
from apple.wallet.cat_wallet.cat_wallet import CATWallet
from apple.wallet.cat_wallet.dao_cat_wallet import DAOCATWallet
from apple.wallet.conditions import Condition, ConditionValidTimes, parse_timelock_info
...
from apple.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
...
from apple.wallet.did_wallet.did_info import DIDCoinData
from apple.wallet.did_wallet.did_wallet import DIDWallet
from apple.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, match_did_puzzle
from apple.wallet.key_val_store import KeyValStore
from apple.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
...
from apple.wallet.notification_manager import NotificationManager
from apple.wallet.outer_puzzles import AssetType
from apple.wallet.payment import Payment
from apple.wallet.puzzle_drivers import PuzzleInfo
from apple.wallet.puzzles.clawback.drivers import generate_clawback_spend_bundle, match_clawback_puzzle
from apple.wallet.puzzles.clawback.metadata import ClawbackMetadata, ClawbackVersion
from apple.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_hash_for_synthetic_public_key,
)
from apple.wallet.sign_coin_spends import sign_coin_spends
from apple.wallet.singleton import create_singleton_puzzle, get_inner_puzzle_from_singleton, get_singleton_id_from_puzzle
from apple.wallet.trade_manager import TradeManager
from apple.wallet.trading.trade_status import TradeStatus
from apple.wallet.transaction_record import TransactionRecord
from apple.wallet.uncurried_puzzle import uncurry_puzzle
from apple.wallet.util.address_type import AddressType
...
from apple.wallet.util.puzzle_decorator import PuzzleDecoratorManager
from apple.wallet.util.query_filter import HashFilter
from apple.wallet.util.transaction_type import CLAWBACK_INCOMING_TRANSACTION_TYPES, TransactionType
from apple.wallet.util.tx_config import TXConfig, TXConfigLoader
from apple.wallet.util.wallet_sync_utils import (
    PeerRequestException,
    fetch_coin_spend_for_coin_state,
    last_change_height_cs,
)
from apple.wallet.util.wallet_types import CoinType, WalletIdentifier, WalletType
from apple.wallet.vc_wallet.cr_cat_drivers import CRCAT, ProofsChecker, construct_pending_approval_state
from apple.wallet.vc_wallet.cr_cat_wallet import CRCATWallet
from apple.wallet.vc_wallet.vc_drivers import VerifiedCredential
from apple.wallet.vc_wallet.vc_store import VCStore
from apple.wallet.vc_wallet.vc_wallet import VCWallet
from apple.wallet.wallet import Wallet
from apple.wallet.wallet_blockchain import WalletBlockchain
from apple.wallet.wallet_coin_record import MetadataTypes, WalletCoinRecord
from apple.wallet.wallet_coin_store import WalletCoinStore
from apple.wallet.wallet_info import WalletInfo
from apple.wallet.wallet_interested_store import WalletInterestedStore
...
from apple.wallet.wallet_puzzle_store import WalletPuzzleStore
from apple.wallet.wallet_retry_store import WalletRetryStore
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSAppleConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ...
    async def spend_clawback_coins(
        ...
        for coin, metadata in clawback_coins.items():
            try:
                ...
                # Remove the clawback hint since it is unnecessary for the APPLE coin
        ...
        if fee > 0:
            apple_tx = await self.main_wallet.create_tandem_apple_tx(
                fee, tx_config, Announcement(coin_spends[0].coin.name(), message)
            )
            assert apple_tx.spend_bundle is not None
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        apple_spam_amount = self.config.get("apple_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if apple_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(apple_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < apple_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSAppleConnection) -> Optional[bytes32]:
        # Get minter DID
