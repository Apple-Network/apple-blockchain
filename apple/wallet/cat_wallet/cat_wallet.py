...
from apple.types.coin_spend import compute_additions_with_cost
...
from apple.util.ints import uint32, uint64, uint128
from apple.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from apple.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, LegacyCATInfo
from apple.wallet.cat_wallet.cat_utils import (
    CAT_MOD,
    SpendableCAT,
...
from apple.wallet.cat_wallet.lineage_store import CATLineageStore
from apple.wallet.coin_selection import select_coins
from apple.wallet.conditions import Condition, ConditionValidTimes, UnknownCondition, parse_timelock_info
...
from apple.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
from apple.wallet.util.curry_and_treehash import calculate_hash_of_quoted_mod_hash, curry_and_treehash
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            apple_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        apple_tx = dataclasses.replace(apple_tx, spend_bundle=spend_bundle, name=spend_bundle.name())
        await self.standard_wallet.push_transaction(apple_tx)
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSAppleConnection, parent_coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def create_tandem_apple_tx(
        self,
        fee: uint64,
        ...
        if fee > amount_to_claim:
            apple_coins = await self.standard_wallet.select_coins(
                fee,
                tx_config.coin_selection_config,
            )
            origin_id = list(apple_coins)[0].name()
            [apple_tx] = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
            ...
            origin_id = list(apple_coins)[0].name()
            selected_amount = sum([c.amount for c in apple_coins])
            [apple_tx] = await self.standard_wallet.generate_signed_transaction(
            ...
            message = None
            for spend in apple_tx.spend_bundle.coin_spends:
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some APPLE as part of this
        regular_apple_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_apple_to_claim = payment_amount

        need_apple_transaction = (fee > 0 or regular_apple_to_claim > 0) and (fee - regular_apple_to_claim != 0)
        ...
        for coin in cat_coins:
            ...
            if first:
                ...
                if need_apple_transaction:
                    if fee > regular_apple_to_claim:
                        apple_tx, _ = await self.create_tandem_apple_tx(
                            fee,
                            uint64(regular_apple_to_claim),
                            tx_config,
                    ...
                    elif regular_apple_to_claim > fee:
                        apple_tx, _ = await self.create_tandem_apple_tx(
                            fee,
                            uint64(regular_apple_to_claim),
                            tx_config,
        ...
        apple_spend_bundle = SpendBundle([], G2Element())
        if apple_tx is not None and apple_tx.spend_bundle is not None:
            apple_spend_bundle = apple_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if apple_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=apple_tx.confirmed_at_height,
                    created_at_time=apple_tx.created_at_time,
                    to_puzzle_hash=apple_tx.to_puzzle_hash,
                    amount=apple_tx.amount,
                    fee_amount=apple_tx.fee_amount,
                    confirmed=apple_tx.confirmed,
                    sent=apple_tx.sent,
                    spend_bundle=None,
                    additions=apple_tx.additions,
                    removals=apple_tx.removals,
                    wallet_id=apple_tx.wallet_id,
                    sent_to=apple_tx.sent_to,
                    trade_id=apple_tx.trade_id,
                    type=apple_tx.type,
                    name=apple_tx.name,
                    memos=[],
                    valid_times=parse_timelock_info(extra_conditions),
