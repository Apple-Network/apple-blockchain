...
from apple.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        apple_tx = dataclasses.replace(apple_tx, spend_bundle=spend_bundle)
    ...
    async def create_tandem_apple_tx(
        ...
        if fee > amount_to_claim:
            apple_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            ...
            if first:
                ...
                if need_apple_transaction:
                    if fee > regular_apple_to_claim:
                        ...
                        apple_tx, _ = await self.create_tandem_apple_tx(
                            fee, uint64(regular_apple_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
