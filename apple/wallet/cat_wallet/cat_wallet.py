...
from apple.wallet.puzzles.genesis_checkers import ALL_LIMITATIONS_PROGRAMS
                ...
                if need_apple_transaction:
                    if fee > regular_apple_to_claim:
                        apple_tx, _ = await self.create_tandem_apple_tx(
                            fee, uint64(regular_apple_to_claim), announcement_to_assert=announcement
                        )
                        ...
                        apple_tx, _ = await self.create_tandem_apple_tx(fee, uint64(regular_apple_to_claim))
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
