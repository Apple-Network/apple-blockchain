...
from apple.util.ints import uint64, uint32, uint8, uint128
...
class DIDWallet:
    ...
    async def create_tandem_apple_tx(
        ...
        apple_coins = await self.standard_wallet.select_coins(fee)
    ...
    async def create_update_spend(self, fee: uint64 = uint64(0)):
        ...
        if fee > 0:
            ...
            apple_tx = await self.create_tandem_apple_tx(fee, Announcement(coin.name(), announcement_to_make))
        else:
