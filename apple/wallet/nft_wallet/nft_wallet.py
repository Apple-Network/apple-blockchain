...
from apple.util.condition_tools import conditions_dict_for_solution, pkm_pairs_for_conditions_dict
from apple.util.hash import std_hash
from apple.util.ints import uint16, uint32, uint64, uint128
from apple.wallet.conditions import Condition, UnknownCondition, parse_timelock_info
from apple.wallet.derivation_record import DerivationRecord
...
from apple.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from apple.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from apple.wallet.nft_wallet.uncurry_nft import NFTCoinData, UncurriedNFT
from apple.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from apple.wallet.payment import Payment
from apple.wallet.puzzle_drivers import PuzzleInfo, Solver
from apple.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from apple.wallet.trading.offer import OFFER_MOD, OFFER_MOD_HASH, NotarizedPayment, Offer
...
from apple.wallet.util.tx_config import CoinSelectionConfig, TXConfig
from apple.wallet.util.wallet_types import WalletType
...
from apple.wallet.wallet_nft_store import WalletNftStore
from apple.wallet.wallet_protocol import GSTOptionalArgs, WalletProtocol
...
class NFTWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSAppleConnection, parent_coin_data: Optional[NFTCoinData]
    ) -> None:
    ...
    async def puzzle_solution_received(self, coin: Coin, data: NFTCoinData, peer: WSAppleConnection) -> None:
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        unsigned_spend_bundle, apple_tx = await self.generate_unsigned_spendbundle(
            payments,
            tx_config,
            fee,
            coins=coins,
        ...
        if apple_tx is not None and apple_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, apple_tx.spend_bundle])
            apple_tx = dataclasses.replace(apple_tx, spend_bundle=None)
        ...
        if apple_tx is not None:
            tx_list.append(apple_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            apple_tx = await self.standard_wallet.create_tandem_apple_tx(
        ...
        return nft_spend_bundle, apple_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'APPLE'} to offer")
    ...
    async def mint_from_did(
        ...
        apple_coins: Optional[Set[Coin]] = None,
        apple_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an apple spend that funds the transactions and pays fees.
        ...
        :param apple_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param apple_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the apple spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an apple coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if apple_coins is None:
            apple_coins = await self.standard_wallet.select_coins(uint64(total_amount), tx_config.coin_selection_config)
        assert len(apple_coins) > 0
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            intermediate_launcher_puz = did_wallet_puzzles.INTERMEDIATE_LAUNCHER_MOD.curry(
                apple.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, mint_number, mint_total
            )
            ...
            launcher_coin = Coin(
                intermediate_launcher_coin.name(), apple.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, amount
            )
            ...
            launcher_cs = CoinSpend(
                launcher_coin, apple.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE, genesis_launcher_solution
            )
        ...
        # Create the apple spend to fund the minting.
        spend_value = sum([coin.amount for coin in apple_coins])
        change: uint64 = uint64(spend_value - total_amount)
        apple_spends = []
        if apple_change_ph is None:
            apple_change_ph = await self.standard_wallet.get_new_puzzlehash()
        apple_payment = Payment(apple_change_ph, change, [apple_change_ph])

        first = True
        for apple_coin in apple_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(apple_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in apple_coins]
                message_list.append(Coin(apple_coin.name(), apple_payment.puzzle_hash, apple_payment.amount).name())
                message: bytes32 = std_hash(b"".join(message_list))

                if len(apple_coins) > 1:
                    apple_announcement: Optional[Set[bytes]] = {message}
                else:
                    apple_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=[apple_payment],
                    fee=fee,
                    coin_announcements=apple_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(apple_coin.name(), message).name()
            ...
            apple_spends.append(CoinSpend(apple_coin, puzzle, solution))
        apple_spend = await self.wallet_state_manager.sign_transaction(apple_spends)
        ...
        did_full_puzzle: Program = apple.wallet.singleton.create_singleton_puzzle(
            innerpuz,
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, apple_spend, *eve_spends])

        tx_record: TransactionRecord = dataclasses.replace(eve_txs[0], spend_bundle=total_spend)
        return [tx_record]

    async def mint_from_apple(
        self,
        metadata_list: List[Dict[str, Any]],
        tx_config: TXConfig,
        target_list: Optional[List[bytes32]] = [],
        ...
        """
        Minting NFTs from a single APPLE spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = apple_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from APPLE to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the APPLE spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the APPLE spend
            ...
            # Annnouncements for eve spend. These are asserted by the apple spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for apple_coin in apple_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=[apple_payment] + primaries,
                    fee=fee,
                    coin_announcements=apple_announcement if len(apple_coins) > 1 else None,
