...
from apple.consensus.blockchain import ReceiveBlockResult
from apple.consensus.constants import ConsensusConstants
from apple.daemon.keychain_proxy import (
...
from apple.util.chunks import chunks
...
from apple.types.weight_proof import WeightProof, SubEpochData
from apple.util.byte_types import hexstr_to_bytes
from apple.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from apple.util.default_root import STANDALONE_ROOT_PATH
from apple.util.ints import uint32, uint64
from apple.util.keychain import KeyringIsLocked, Keychain
from apple.util.path import mkdir, path_from_root
from apple.wallet.util.new_peak_queue import NewPeakQueue, NewPeakQueueTypes, NewPeakItem
...
class WalletNode:
    ...
    def on_disconnect(self, peer: WSAppleConnection):
    ...
    async def on_connect(self, peer: WSAppleConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def is_peer_synced(
        self, peer: WSAppleConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSAppleConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSAppleConnection]:
        if self.server is None:
            return None
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peer: Optional[WSAppleConnection] = self.get_full_node_peer()
        if peer is None:
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSAppleConnection):
        if self.wallet_state_manager is None:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSAppleConnection, peak: HeaderBlock
    ...
    async def fetch_puzzle_solution(self, peer: WSAppleConnection, height: uint32, coin: Coin) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], fork_height: Optional[uint32] = None, peer: Optional[WSAppleConnection] = None
    ) -> List[CoinState]:
        assert self.server is not None
        all_nodes = self.server.connection_by_type[NodeType.FULL_NODE]
        if len(all_nodes.keys()) == 0:
    ...
    async def fetch_children(
        self, peer: WSAppleConnection, coin_name: bytes32, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
