...
from apple.consensus.blockchain import AddBlockResult
from apple.consensus.constants import ConsensusConstants
from apple.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
from apple.full_node.full_node_api import FullNodeAPI
from apple.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from apple.protocols.protocol_message_types import ProtocolMessageTypes
from apple.protocols.wallet_protocol import (
    CoinState,
    CoinStateUpdate,
    NewPeakWallet,
...
from apple.rpc.rpc_server import StateChangedProtocol, default_get_connections
from apple.server.node_discovery import WalletPeers
...
from apple.util.config import (
    WALLET_PEERS_PATH_KEY_DEPRECATED,
    lock_and_load_config,
    process_config_start_method,
    save_config,
)
from apple.util.db_wrapper import manage_connection
from apple.util.errors import KeychainIsEmpty, KeychainIsLocked, KeychainKeyNotFound, KeychainProxyConnectionFailure
from apple.util.hash import std_hash
from apple.util.ints import uint16, uint32, uint64, uint128
from apple.util.keychain import Keychain
from apple.util.misc import to_batches
...
from apple.wallet.puzzles.clawback.metadata import AutoClaimSettings
from apple.wallet.transaction_record import TransactionRecord
from apple.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from apple.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
...
from apple.wallet.wallet_state_manager import WalletStateManager
from apple.wallet.wallet_weight_proof_handler import WalletWeightProofHandler, get_wp_fork_point
...
class WalletNode:
    ...
    def get_cache_for_peer(self, peer: WSAppleConnection) -> PeerRequestCache:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'apple keys' program.")
            return None
    ...
    def initialize_wallet_peers(self) -> None:
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns.applecoin.in"]),
    ...
    def on_disconnect(self, peer: WSAppleConnection) -> None:
    ...
    async def on_connect(self, peer: WSAppleConnection) -> None:
        if self._wallet_state_manager is None:
            return None
    ...
    def is_trusted(self, peer: WSAppleConnection) -> bool:
        return self.server.is_trusted_peer(peer, self.config.get("trusted_peers", {}))

    async def state_update_received(self, request: CoinStateUpdate, peer: WSAppleConnection) -> None:
    ...
    def get_full_node_peer(self) -> WSAppleConnection:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSAppleConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            raise ValueError("No peer connected")
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSAppleConnection]:
        """
        ...
        synced_and_trusted: List[WSAppleConnection] = []
        synced: List[WSAppleConnection] = []
        trusted: List[WSAppleConnection] = []
        neither: List[WSAppleConnection] = []
        all_nodes: List[WSAppleConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def get_timestamp_for_height_from_peer(self, height: uint32, peer: WSAppleConnection) -> Optional[uint64]:
        """
    ...
    async def new_peak_wallet(self, new_peak: NewPeakWallet, peer: WSAppleConnection) -> None:
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(
        self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSAppleConnection
    ) -> None:
    ...
    async def new_peak_from_untrusted(self, new_peak_hb: HeaderBlock, peer: WSAppleConnection) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSAppleConnection) -> None:
    ...
    async def sync_from_untrusted_close_to_peak(self, new_peak_hb: HeaderBlock, peer: WSAppleConnection) -> bool:
        async with self.wallet_state_manager.lock:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSAppleConnection) -> int:
    ...
    async def fetch_and_update_weight_proof(self, peer: WSAppleConnection, peak: HeaderBlock) -> int:
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSAppleConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSAppleConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSAppleConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
