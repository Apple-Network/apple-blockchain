...
from apple.full_node.mempool_check_conditions import get_name_puzzle_conditions
...
from apple.wallet.secret_key_store import SecretKeyStore
...
class Wallet:
    ...
    async def create_spend_bundle_relative_apple(self, apple_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if apple_amount < 0:
            utxos = await self.select_coins(uint64(abs(apple_amount)), exclude)
        else:
        ...
        apple_amount = spend_value + apple_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(apple_amount), "memos": []}
                ]
