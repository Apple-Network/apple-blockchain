...
# the output will be printed to your apple root path, e.g. ~/.apple/mainnet/profile/
# to analyze the profile, run:

#   python apple/utils/profiler.py ~/.apple/mainnet/profile | less -r

# this will print CPU usage of the apple full node main thread at 1 second increments.
# find a time window of interest and analyze the profile file (which are in pstats format).

# for example:

#   python apple/utils/profiler.py ~/.apple/mainnet/profile 10 20
...
if __name__ == "__main__":
    ...
    def analyze_slot_range(profile_dir: pathlib.Path, first: int, last: int) -> None:
        ...
        output_file = "apple-hotspot-%d" % first
        if first < last:
            output_file += "-%d" % last
