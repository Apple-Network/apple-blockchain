...
from apple.data_layer.data_layer_errors import LauncherCoinNotFoundError, OfferIntegrityError
from apple.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from apple.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from apple.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
...
from apple.wallet.util.merkle_utils import _simplify_merkle_proof
from apple.wallet.util.transaction_type import TransactionType
from apple.wallet.util.tx_config import CoinSelectionConfig, TXConfig, TXConfigLoader
from apple.wallet.util.wallet_sync_utils import fetch_coin_spend, fetch_coin_spend_for_coin_state
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSAppleConnection) -> CoinState:
    ...
    async def delete_mirror(
        ...
        if excess_fee > 0:
            [apple_tx] = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
                tx_config,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, apple_tx.spend_bundle])
                ),
                dataclasses.replace(apple_tx, spend_bundle=None),
            ]
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSAppleConnection, coin_data: Optional[object]) -> None:
