...
from apple.data_layer.data_layer_errors import LauncherCoinNotFoundError
from apple.data_layer.data_layer_wallet import DataLayerWallet
from apple.pools.pool_puzzles import create_p2_singleton_puzzle, POOL_OUTER_MOD_HASH
from apple.pools.pool_wallet import PoolWallet
from apple.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
...
from apple.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
from apple.rpc.util import tx_endpoint
from apple.server.outbound_message import NodeType, make_msg
from apple.server.ws_connection import WSAppleConnection
from apple.simulator.simulator_protocol import FarmNewBlockProtocol
...
from apple.types.signing_mode import CHIP_0002_SIGN_MESSAGE_PREFIX, SigningMode
from apple.types.spend_bundle import SpendBundle, estimate_fees
from apple.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from apple.util.byte_types import hexstr_to_bytes
from apple.util.config import load_config, str2bool
from apple.util.errors import KeychainIsLocked
from apple.util.ints import uint16, uint32, uint64
from apple.util.keychain import bytes_to_mnemonic, generate_mnemonic
from apple.util.misc import UInt32Range
from apple.util.path import path_from_root
from apple.util.streamable import Streamable, streamable
from apple.util.ws_message import WsRpcMessage, create_payload_dict
from apple.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from apple.wallet.cat_wallet.cat_info import CRCATInfo
from apple.wallet.cat_wallet.cat_wallet import CATWallet
from apple.wallet.cat_wallet.dao_cat_info import LockedCoinInfo
from apple.wallet.cat_wallet.dao_cat_wallet import DAOCATWallet
from apple.wallet.conditions import Condition
from apple.wallet.dao_wallet.dao_info import DAORules
from apple.wallet.dao_wallet.dao_utils import (
...
from apple.wallet.dao_wallet.dao_wallet import DAOWallet
from apple.wallet.derive_keys import (
    MAX_POOL_WALLETS,
...
from apple.wallet.did_wallet import did_wallet_puzzles
from apple.wallet.did_wallet.did_info import DIDCoinData, DIDInfo
from apple.wallet.did_wallet.did_wallet import DIDWallet
from apple.wallet.did_wallet.did_wallet_puzzles import (
    DID_INNERPUZ_MOD,
    did_program_to_metadata,
    match_did_puzzle,
    metadata_to_program,
)
from apple.wallet.nft_wallet import nft_puzzles
from apple.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTInfo
from apple.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from apple.wallet.nft_wallet.nft_wallet import NFTWallet
from apple.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from apple.wallet.notification_store import Notification
from apple.wallet.outer_puzzles import AssetType
from apple.wallet.payment import Payment
from apple.wallet.puzzle_drivers import PuzzleInfo, Solver
from apple.wallet.puzzles.clawback.metadata import AutoClaimSettings, ClawbackMetadata
from apple.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
from apple.wallet.singleton import (
    SINGLETON_LAUNCHER_PUZZLE_HASH,
    create_singleton_puzzle,
    get_inner_puzzle_from_singleton,
)
from apple.wallet.trade_record import TradeRecord
from apple.wallet.trading.offer import Offer
from apple.wallet.transaction_record import TransactionRecord
from apple.wallet.uncurried_puzzle import uncurry_puzzle
from apple.wallet.util.address_type import AddressType, is_valid_address
from apple.wallet.util.compute_hints import compute_spend_hints_and_additions
from apple.wallet.util.compute_memos import compute_memos
from apple.wallet.util.query_filter import HashFilter, TransactionTypeFilter
from apple.wallet.util.transaction_type import CLAWBACK_INCOMING_TRANSACTION_TYPES, TransactionType
from apple.wallet.util.tx_config import DEFAULT_TX_CONFIG, CoinSelectionConfig, CoinSelectionConfigLoader, TXConfig
from apple.wallet.util.wallet_sync_utils import fetch_coin_spend_for_coin_state
from apple.wallet.util.wallet_types import CoinType, WalletType
from apple.wallet.vc_wallet.cr_cat_drivers import ProofsChecker
from apple.wallet.vc_wallet.cr_cat_wallet import CRCATWallet
from apple.wallet.vc_wallet.vc_store import VCProofs
from apple.wallet.vc_wallet.vc_wallet import VCWallet
from apple.wallet.wallet import Wallet
from apple.wallet.wallet_coin_record import WalletCoinRecord
from apple.wallet.wallet_coin_store import CoinRecordOrder, GetCoinRecords, unspent_range
from apple.wallet.wallet_info import WalletInfo
from apple.wallet.wallet_node import WalletNode
from apple.wallet.wallet_protocol import WalletProtocol
...
class WalletRpcApi:
    ...
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "apple_wallet"
    ...
    async def get_latest_singleton_coin_spend(
        self, peer: WSAppleConnection, coin_id: bytes32, latest: bool = True
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("apple_target_address")
        pool_target = config["pool"].get("apple_target_address")
    ...
    async def create_new_wallet(
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                if request.get("test", False):
                    ...
                    raise ValueError(
                        ...
                        " Please use the CAT Admin Tool @ https://github.com/Apple-Network/CAT-admin-tool instead."
                    )
            ...
            if request["mode"] == "new":
                ...
                from apple.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def verify_signature(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        # signatures made by `apple keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
    ...
    async def get_offer_summary(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        from apple.util.bech32m import bech32_decode, convertbits
        from apple.wallet.util.puzzle_compression import OFFER_MOD_OLD, decompress_object_with_puzzles
    ...
    async def cancel_offers(
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "apple")
        ...
        key = None
        if asset_id is not None and asset_id != "apple":
    ...
    async def did_get_info(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.APPLE.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(
        ...
        apple_coin_list = request.get("apple_coins", None)
        apple_coins = None
        if apple_coin_list:
            apple_coins = {Coin.from_json_dict(apple_coin) for apple_coin in apple_coin_list}
        apple_change_target = request.get("apple_change_target", None)
        if apple_change_target is not None:
            if apple_change_target[:2] == "apple":
                apple_change_ph = decode_puzzle_hash(apple_change_target)
            else:
                apple_change_ph = bytes32(hexstr_to_bytes(apple_change_target))
        else:
            apple_change_ph = None
        ...
        if mint_from_did:
            txs = await nft_wallet.mint_from_did(
                ...
                apple_coins=apple_coins,
                apple_change_ph=apple_change_ph,
            ...
            txs = await nft_wallet.mint_from_apple(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
