...
from apple.plot_sync.delta import Delta
from apple.plot_sync.receiver import Receiver
from apple.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
from apple.protocols import farmer_protocol, harvester_protocol
from apple.protocols.pool_protocol import (
...
from apple.server.server import AppleServer, ssl_context_for_root
from apple.server.ws_connection import WSAppleConnection
from apple.ssl.create_ssl import get_mozilla_ca_crt
from apple.types.blockchain_format.proof_of_space import ProofOfSpace
from apple.types.blockchain_format.sized_bytes import bytes32
from apple.util.bech32m import decode_puzzle_hash
from apple.util.byte_types import hexstr_to_bytes
from apple.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from apple.util.errors import KeychainProxyConnectionFailure
from apple.util.hash import std_hash
from apple.util.ints import uint8, uint16, uint32, uint64
from apple.util.keychain import Keychain
from apple.util.logging import TimedDuplicateFilter
...
from apple.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'apple keys generate' or open the UI."
        try:
        ...
        if "apple_target_address" not in self.config:
            self.config = config["farmer"]
        if "apple_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "apple_target_address" not in self.config or "apple_target_address" not in self.pool_config:
            log.debug("apple_target_address missing in the config")
            return False

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["apple_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["apple_target_address"]
