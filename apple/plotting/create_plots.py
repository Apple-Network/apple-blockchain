...
from apple.plotting.util import stream_plot_info_ph, stream_plot_info_pk
...
from apple.util.config import config_path_for_filename, load_config
from apple.util.keychain import Keychain
from apple.util.path import mkdir
from apple.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk, master_sk_to_pool_sk
...
class PlotKeysResolver:
    ...
    async def get_farmer_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'apple keys add', 'apple keys generate' or provide a public key with -f"
            )
    ...
    async def get_pool_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'apple keys add', 'apple keys generate' or provide a public key with -p"
            )
