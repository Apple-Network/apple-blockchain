...
from apple.util.db_version import set_db_version
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("apple_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("apple_target_address"):
        ...
        if "apple_target_address" not in config["farmer"]:
            print(
                f"Setting the apple destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
...
def apple_init(
    ...
    *,
    ...
    """
    apple_root = os.environ.get("APPLE_ROOT", None)
    if apple_root is not None:
        print(f"APPLE_ROOT is set to {apple_root}")
