...
from apple.util.ssl_check import (
...
from apple.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'apple keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "apple_target_address" not in config["farmer"]
    stop_searching_for_pool = "apple_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("apple_target_address") or all_targets[-2] == config["farmer"].get(
                "apple_target_address"
            ):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("apple_target_address") or all_targets[-2] == config["pool"].get(
                "apple_target_address"
            ):
    ...
    if "apple_target_address" not in config["farmer"]:
        print(
            f"Setting the apple destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["apple_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["apple_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we might not have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['apple_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "apple_target_address" not in config["pool"]:
        print(f"Setting the apple destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["apple_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["apple_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we might not have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['apple_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the APPLE destination addresses, edit the `apple_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    apple_ca_crt, apple_ca_key = get_apple_ca_crt_key()
    apple_ca_crt_path = ca_dir / "apple_ca.crt"
    apple_ca_key_path = ca_dir / "apple_ca.key"
    write_ssl_cert_and_key(apple_ca_crt_path, apple_ca_crt, apple_ca_key_path, apple_ca_key)
    ...
    apple_ca_crt, apple_ca_key = get_apple_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, apple_ca_crt, apple_ca_key, False, overwrite=False)
...
def init(
    ...
    if create_certs is not None:
        ...
        return apple_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)


def apple_version_number() -> Tuple[str, str, str, str]:
...
def apple_minor_release_number():
    res = int(apple_version_number()[2])
    print(f"Install release number: {res}")
    return res


def apple_full_version_str() -> str:
    major, minor, patch, dev = apple_version_number()
...
def apple_init(
    ...
    *,
    ...
    """
    if os.environ.get("APPLE_ROOT", None) is not None:
        print(
            f"warning, your APPLE_ROOT is set to {os.environ['APPLE_ROOT']}. "
            f"Please unset the environment variable and run apple init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Apple directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if APPLE_ROOT is set, or if user has run apple init twice
        # before a new update.
        if testnet:
            configure(
                root_path,
    ...
    create_default_apple_config(root_path)
    if testnet:
        configure(
            root_path,
            set_farmer_peer="",
    ...
    print("")
    print("To see your keys, run 'apple keys show --show-mnemonic-seed'")
