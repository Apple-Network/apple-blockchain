...
from apple.cmds.beta import beta_cmd
from apple.cmds.completion import completion
from apple.cmds.configure import configure_cmd
from apple.cmds.dao import dao_cmd
from apple.cmds.data import data_cmd
from apple.cmds.db import db_cmd
from apple.cmds.dev import dev_cmd
from apple.cmds.farm import farm_cmd
from apple.cmds.init import init_cmd
from apple.cmds.keys import keys_cmd
from apple.cmds.netspace import netspace_cmd
from apple.cmds.passphrase import passphrase_cmd
from apple.cmds.peer import peer_cmd
from apple.cmds.plotnft import plotnft_cmd
from apple.cmds.plots import plots_cmd
from apple.cmds.plotters import plotters_cmd
...
from apple.cmds.wallet import wallet_cmd
from apple.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from apple.util.errors import KeychainCurrentPassphraseIsInvalid
from apple.util.keychain import Keychain, set_keys_root_path
from apple.util.ssl_check import check_ssl

CONTEXT_SETTINGS = dict(help_option_names=["-h", "--help"])


@click.group(
    help=f"\n  Manage apple blockchain infrastructure ({__version__})\n",
    epilog="Try 'apple start node', 'apple netspace -d 192', or 'apple show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        ...
        from apple.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
@cli.command("version", help="Show apple version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", help="Runs apple daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by apple start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio

    from apple.daemon.server import async_run_daemon
