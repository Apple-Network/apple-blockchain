...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
def recover_pool_nft(wallet_rpc_port: Optional[int], launcher_id: str, contract_address: Optional[str], fingerprint: int):
    import asyncio
    from .wallet_funcs import execute_with_wallet, recover_pool_nft

    extra_params = {"launcher_id": launcher_id, "contract_address": contract_address}
