...
from apple.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate
from apple.rpc.data_layer_rpc_client import DataLayerRpcClient
from apple.rpc.farmer_rpc_client import FarmerRpcClient
from apple.rpc.full_node_rpc_client import FullNodeRpcClient
from apple.rpc.harvester_rpc_client import HarvesterRpcClient
...
from apple.types.mempool_submission_status import MempoolSubmissionStatus
from apple.util.config import load_config
from apple.util.default_root import DEFAULT_ROOT_PATH
from apple.util.errors import CliRpcConnectionError
from apple.util.ints import uint16, uint64
from apple.util.keychain import KeyData
from apple.util.streamable import Streamable, streamable
from apple.wallet.transaction_record import TransactionRecord
from apple.wallet.util.tx_config import CoinSelectionConfig, CoinSelectionConfigLoader, TXConfig, TXConfigLoader
...
def transaction_status_msg(fingerprint: int, tx_id: bytes32) -> str:
    return f"Run 'apple wallet get_transaction -f {fingerprint} -tx 0x{tx_id}' to get status"
    ...
    try:
        if fingerprint is not None:
            ...
            if len(fingerprints) == 0:
                raise CliRpcConnectionError("No keys loaded. Run 'apple keys generate' or import a key")
        ...
        click.option(
            ...
            click.option(
                ...
                click.option(
                    ...
                    "amounts_to_exclude",
                    multiple=True,
                    help="Exclude any coins with this APPLE or CAT amount from being included.",
                )(func)
            )
        )
    )
