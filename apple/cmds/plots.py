...
from apple.plotting.util import add_plot_directory, validate_plot_size

log = logging.getLogger(__name__)


def show_plots(root_path: Path) -> None:
    from apple.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'apple plots add -d [dir]' and remove with"
        + " 'apple plots remove -d [dir]'"
        + " Scan and check plots with 'apple plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context) -> None:
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'apple init'")
...
def create_cmd(
    ...
    from apple.plotting.create_plots import create_plots, resolve_plot_keys
    from apple.plotting.util import Params
...
def check_cmd(
    ...
    from apple.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str) -> None:
    from apple.plotting.util import remove_plot_directory
