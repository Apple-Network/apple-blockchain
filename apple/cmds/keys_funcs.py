...
from apple.cmds.passphrase_funcs import obtain_current_passphrase
from apple.consensus.coinbase import create_puzzlehash_for_pk
from apple.types.signing_mode import SigningMode
from apple.util.bech32m import encode_puzzle_hash
from apple.util.config import load_config
from apple.util.errors import KeychainException
from apple.util.file_keyring import MAX_LABEL_LENGTH
from apple.util.ints import uint32
from apple.util.keychain import Keychain, KeyData, bytes_to_mnemonic, generate_mnemonic, mnemonic_to_seed
...
def generate_and_print() -> str:
    ...
    print(mnemonic)
    print("Note that this key has not been added to the keychain. Run apple keys add")
    return mnemonic
...
def show_all_key_labels() -> None:
    ...
    if len(keys) == 0:
        sys.exit("No keys are present in the keychain. Generate them with 'apple keys generate'")
...
def derive_sk_from_hd_path(master_sk: PrivateKey, hd_path_root: str) -> Tuple[PrivateKey, str]:
    ...
    from apple.wallet.derive_keys import _derive_path, _derive_path_unhardened
