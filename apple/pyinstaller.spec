...
ROOT = pathlib.Path(importlib.import_module("apple").__file__).absolute().parent.parent
...
def solve_name_collision_problem(analysis):
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("apple/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("apple-blockchain"))[0]
...
# TODO: collapse all these entry points into one `apple_exec` entrypoint that accepts the server as a parameter

entry_points = ["apple.cmds.apple"] + [f"apple.server.start_{s}" for s in SERVERS]
...
binaries = [
    (
        f"{ROOT}/madmax/apple_plot",
        "madmax"
    ),
    (
        f"{ROOT}/madmax/apple_plot_k34",
        "madmax"
    )
]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "apple.util.bip39"])
...
if THIS_IS_WINDOWS:
    ...
    binaries = [
        ...
        (
            f"{ROOT}\\madmax\\apple_plot.exe",
            "madmax"
        ),
        (
            f"{ROOT}\\madmax\\apple_plot_k34.exe",
            "madmax"
        ),
        (
...
datas.append((f"{ROOT}/apple/util/english.txt", "apple/util"))
datas.append((f"{ROOT}/apple/util/initial-config.yaml", "apple/util"))
datas.append((f"{ROOT}/apple/wallet/puzzles/*.hex", "apple/wallet/puzzles"))
datas.append((f"{ROOT}/apple/ssl/*", "apple/ssl"))
...
add_binary("apple", f"{ROOT}/apple/cmds/apple.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/apple/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/apple/server/start_{server}.py", COLLECT_ARGS)

add_binary("start_crawler", f"{ROOT}/apple/seeder/start_crawler.py", COLLECT_ARGS)
add_binary("start_seeder", f"{ROOT}/apple/seeder/dns_server.py", COLLECT_ARGS)
