...
import apple.server.ws_connection as ws  # lgtm [py/import-and-import-from]
...
from apple.consensus.blockchain import Blockchain, ReceiveBlockResult, StateChangeSummary
...
from apple.full_node.lock_queue import LockQueue, LockClient
...
from apple.util.errors import ConsensusError, Err, ValidationError
...
class FullNode:
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSAppleConnection):
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[ws.WSAppleConnection] = None
    ):
        """
    ...
    async def on_connect(self, connection: ws.WSAppleConnection):
        """
    ...
    def on_disconnect(self, connection: ws.WSAppleConnection):
    ...
    def remove_subscriptions(self, peer: ws.WSAppleConnection):
    ...
    async def sync_from_fork_point(
        ...
        async def fetch_block_batches(batch_queue: asyncio.Queue, peers_with_peak: List[ws.WSAppleConnection]):
            try:
        ...
        batch_queue: asyncio.Queue[Tuple[ws.WSAppleConnection, List[FullBlock]]] = asyncio.Queue(maxsize=buffer_size)
    ...
    async def update_wallets(
        ...
        for peer, changes in changes_for_peer.items():
            if peer not in self.server.all_connections:
                continue
            ws_peer: ws.WSAppleConnection = self.server.all_connections[peer]
    ...
    async def peak_post_processing(
        ...
        block: FullBlock,
        state_change_summary: StateChangeSummary,
        peer: Optional[ws.WSAppleConnection],
    ...
    async def respond_block(
        ...
        peer: Optional[ws.WSAppleConnection] = None,
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSAppleConnection] = None
    ) -> Optional[Message]:
...
async def node_next_block_check(
    peer: ws.WSAppleConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
