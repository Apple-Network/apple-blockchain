...
async def _apple_create_server(
    cls: Any,
    protocol_factory: _ProtocolFactory,
    host: Any,
    port: Any,
...
class AppleSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _apple_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class AppleProactor(IocpProactor):
        ...
        async def _apple_accept_loop(self, listener: socket.socket) -> Tuple[socket.socket, Tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._apple_accept(listener)
        ...
        def _apple_accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            ...
            async def accept_coro(self: AppleProactor, future: asyncio.Future[object], conn: socket.socket) -> None:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            coro = self._apple_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class AppleProactorEventLoop(ProactorEventLoop):
...
class ApplePolicy(asyncio.DefaultEventLoopPolicy):
    def new_event_loop(self) -> asyncio.AbstractEventLoop:
        # overriding https://github.com/python/cpython/blob/v3.11.0/Lib/asyncio/events.py#L689-L695
        if sys.platform == "win32":
            loop_factory = AppleProactorEventLoop
        else:
            loop_factory = AppleSelectorEventLoop

        return loop_factory()


def set_apple_policy(connection_limit: int) -> None:
    ...
    asyncio.set_event_loop_policy(ApplePolicy())
