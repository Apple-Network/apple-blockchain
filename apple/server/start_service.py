...
from apple.cmds.init_funcs import apple_full_version_str
from apple.daemon.server import service_launch_lock_path
from apple.rpc.rpc_server import RpcApiProtocol, RpcServer, RpcServiceProtocol, start_rpc_server
from apple.server.api_protocol import ApiProtocol
from apple.server.apple_policy import set_apple_policy
from apple.server.outbound_message import NodeType
from apple.server.server import AppleServer
from apple.server.ssl_context import apple_ssl_ca_paths, private_ssl_ca_paths
from apple.server.upnp import UPnP
from apple.server.ws_connection import WSAppleConnection
from apple.types.peer_info import PeerInfo, UnresolvedPeerInfo
...
from apple.util.network import resolve
...
class Service(Generic[_T_RpcServiceProtocol, _T_ApiProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSAppleConnection], Awaitable[None]]] = None,
        ...
        self._log.info(f"apple-blockchain version: {apple_full_version_str()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        apple_ca_crt, apple_ca_key = apple_ssl_ca_paths(root_path, self.config)
        ...
        self._server = AppleServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def manage(self, *, start: bool = True) -> AsyncIterator[None]:
        ...
        async with contextlib.AsyncExitStack() as async_exit_stack:
            try:
                ...
                self._log.info("Waiting for AppleServer to be closed")
    ...
    async def setup_process_global_state(self, signal_handlers: SignalHandlers) -> None:
        ...
        proctitle_name = f"apple_{self._service_name}"
...
def async_run(coro: Coroutine[object, object, T], connection_limit: Optional[int] = None) -> T:
    with log_exceptions(log=log, message="fatal uncaught exception"):
        if connection_limit is not None:
            set_apple_policy(connection_limit)
