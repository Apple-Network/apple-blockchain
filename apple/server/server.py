...
from apple.protocols.protocol_state_machine import message_requires_reply
from apple.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
from apple.protocols.shared_protocol import protocol_version
from apple.server.introducer_peers import IntroducerPeers
from apple.server.outbound_message import Message, NodeType
from apple.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from apple.util.network import is_localhost, is_in_network
from apple.util.ssl import verify_ssl_certs_and_keys
...
class AppleServer:
    def __init__(
        ...
        apple_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSAppleConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSAppleConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.apple_ca_crt_path, self.apple_ca_key_path = apple_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSAppleConnection] = []
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSAppleConnection] = None
        try:
            connection = WSAppleConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSAppleConnection, on_connect=None):
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.apple_ca_crt_path, self.apple_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSAppleConnection] = None
        try:
    ...
    def connection_closed(self, connection: WSAppleConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSAppleConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSAppleConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSAppleConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSAppleConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSAppleConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def is_trusted_peer(self, peer: WSAppleConnection, trusted_peers: Dict) -> bool:
