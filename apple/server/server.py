...
from apple.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS
from apple.protocols.shared_protocol import protocol_version
from apple.server.api_protocol import ApiProtocol
...
from apple.server.ws_connection import ConnectionCallback, WSAppleConnection
...
from apple.util.network import WebServer, is_in_network, is_localhost, is_trusted_peer
...
class AppleServer:
    ...
    all_connections: Dict[bytes32, WSAppleConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        apple_ca_crt_path, apple_ca_key_path = apple_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                ca_cert=apple_ca_crt_path,
                ca_key=apple_ca_key_path,
    ...
    async def start(
        ...
        if self.webserver is not None:
            raise RuntimeError("AppleServer already started")
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        try:
            connection = WSAppleConnection.create(
    ...
    async def connection_added(
        self, connection: WSAppleConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    def connection_closed(self, connection: WSAppleConnection, ban_time: int, closed_connection: bool = False) -> None:
    ...
    def is_trusted_peer(self, peer: WSAppleConnection, trusted_peers: Dict[str, Any]) -> bool:
