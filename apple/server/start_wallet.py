...
from apple.rpc.wallet_rpc_api import WalletRpcApi
from apple.server.outbound_message import NodeType
from apple.server.start_service import RpcInfo, Service, async_run
from apple.util.apple_logging import initialize_service_logging
from apple.util.config import get_unresolved_peer_infos, load_config, load_config_cli
...
from apple.util.task_timing import maybe_manage_task_instrumentation
...
async def async_main() -> int:
    ...
    if local_test is True:
        from apple.simulator.block_tools import test_constants
...
def main() -> int:
    ...
    with maybe_manage_task_instrumentation(enable=os.environ.get("APPLE_INSTRUMENT_WALLET") is not None):
