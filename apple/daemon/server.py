...
from apple.cmds.init_funcs import check_keys, apple_full_version_str, apple_init
from apple.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from apple.consensus.coinbase import create_puzzlehash_for_pk
from apple.daemon.keychain_server import KeychainServer, keychain_commands
from apple.daemon.windows_signal import kill
from apple.plotters.plotters import get_available_plotters
from apple.plotting.util import add_plot_directory
from apple.server.server import ssl_context_for_server
from apple.util.bech32m import encode_puzzle_hash
from apple.util.beta_metrics import BetaMetricsLogger
from apple.util.apple_logging import initialize_service_logging
from apple.util.config import load_config
from apple.util.errors import KeychainCurrentPassphraseIsInvalid
from apple.util.ints import uint32
from apple.util.json_util import dict_to_json_str
from apple.util.keychain import Keychain, KeyData, passphrase_requirements, supports_os_passphrase_storage
from apple.util.lock import Lockfile, LockfileError
from apple.util.log_exceptions import log_exceptions
from apple.util.misc import SignalHandlers
from apple.util.network import WebServer
from apple.util.service_groups import validate_service
from apple.util.setproctitle import setproctitle
from apple.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Apple.")
    quit()
...
if getattr(sys, "frozen", False):
    name_map = {
        "apple": "apple",
        "apple_data_layer": "apple_data_layer",
        "apple_data_layer_http": "apple_data_layer_http",
        "apple_wallet": "apple_wallet",
        "apple_full_node": "apple_full_node",
        "apple_harvester": "apple_harvester",
        "apple_farmer": "apple_farmer",
        "apple_introducer": "apple_introducer",
        "apple_timelord": "apple_timelord",
        "apple_timelord_launcher": "timelord_launcher",
        "apple_full_node_simulator": "apple_simulator",
        "apple_seeder": "apple_seeder",
        "apple_crawler": "apple_crawler",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Apple will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["apple", "plotters", plotter]

        if plotter == "bladebit":
            # plotter command must be either
            # 'apple plotters bladebit ramplot' or 'apple plotters bladebit diskplot'
            plot_type = request["plot_type"]
...
def launch_plotter(
    ...
    # we need to pass on the possibly altered APPLE_ROOT
    os.environ["APPLE_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with APPLE_ROOT: {os.environ['APPLE_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in apple_init
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"apple-blockchain version: {apple_full_version_str()}")
