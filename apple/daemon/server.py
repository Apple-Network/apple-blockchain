...
from apple.cmds.init_funcs import check_keys, apple_init
...
from apple.server.server import ssl_context_for_root, ssl_context_for_server
from apple.ssl.create_ssl import get_mozilla_ca_crt
from apple.util.apple_logging import initialize_logging
from apple.util.config import load_config
from apple.util.json_util import dict_to_json_str
from apple.util.keychain import (
    Keychain,
...
if getattr(sys, "frozen", False):
    name_map = {
        "apple": "apple",
        "apple_wallet": "start_wallet",
        "apple_full_node": "start_full_node",
        "apple_harvester": "start_harvester",
        "apple_farmer": "start_farmer",
        "apple_introducer": "start_introducer",
        "apple_timelord": "start_timelord",
        "apple_timelord_launcher": "timelord_launcher",
        "apple_full_node_simulator": "start_simulator",
        "apple_seeder": "start_seeder",
        "apple_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        try:
            if Keychain.master_passphrase_is_valid(key, force_reload=True):
                ...
                try:
                    ...
                    self.log.exception("Failed to migrate keys silently. Run `apple keys migrate` manually.")
    ...
    def _applepos_plotting_command_args(self, request: Any, ignoreCount: bool) -> List[str]:
    ...
    async def start_plotting(self, request: Dict[str, Any]):
        ...
        plotter = request.get("plotter", "applepos")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with APPLE_ROOT: {os.environ['APPLE_ROOT']}")
    ...
    if service_command == "apple_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
