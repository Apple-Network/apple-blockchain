...
# Gets the version from the result of "apple version"
...
def get_apple_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["apple", "version"], capture_output=True)
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_apple_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/apple-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/apple-blockchain-gui/packages/gui/package.json")
