...
if [ ! "$1" ]; then
  echo "This script requires either amd64 of arm64 as an argument"
	...
	DIR_NAME="apple-blockchain-linux-x64"
else
	#PLATFORM="$1"
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create apple-blockchain-$APPLE_INSTALLER_VERSION.rpm"

	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/Apple-Network/apple-blockchain/issues/3846
  ...
  --arch "$REDHAT_PLATFORM" --options.version $APPLE_INSTALLER_VERSION \
  --license ../LICENSE --options.bin apple-blockchain --options.name apple-blockchain
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
