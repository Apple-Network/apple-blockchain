...
export APPLE_INSTALLER_VERSION
...
pip install j2cli
CLI_DEB_BASE="apple-blockchain-cli_$APPLE_INSTALLER_VERSION-1_$PLATFORM"
mkdir -p "dist/$CLI_DEB_BASE/opt/apple"
...
cp -r dist/daemon/* "dist/$CLI_DEB_BASE/opt/apple/"

ln -s ../../opt/apple/apple "dist/$CLI_DEB_BASE/usr/bin/apple"
...
if [ "$PLATFORM" = "arm64" ]; then
  ...
  echo USE_SYSTEM_FPM=true npx electron-builder build --linux deb --arm64 \
    ...
    --config.deb.packageName="apple-blockchain"
...
GUI_DEB_NAME=apple-blockchain_${APPLE_INSTALLER_VERSION}_${PLATFORM}.deb
mv "dist/${PRODUCT_NAME}-${APPLE_INSTALLER_VERSION}.deb" "../../../build_scripts/dist/${GUI_DEB_NAME}"
