...
LAST_EXIT_CODE=$?
ls -l dist/mac*/apple.app/Contents/Resources/app.asar
...
if [ "$(arch)" = "arm64" ]; then
  mv dist/"${DMG_NAME}" dist/apple-"${APPLE_INSTALLER_VERSION}"-arm64.dmg
...
# xcrun notarytool submit --wait --apple-id username --password password --team-id team-id Apple-0.1.X.dmg
