...
git submodule update --init apple-blockchain-gui
...
# Other than `apple-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @apple-network packages to make cache slim"
...
# Remove `packages/gui/node_modules/@apple-network` because it causes an error on later `electron-packager` command
#Remove-Item "@apple-network" -Recurse -Force
