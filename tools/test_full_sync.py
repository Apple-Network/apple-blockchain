...
from apple.simulator.block_tools import make_unfinished_block
...
async def run_sync_test(
    ...
    with tempfile.TemporaryDirectory() as root_dir:
        ...
        apple_init(root_path, should_check_keys=False, v1_db=(db_version == 1))
        ...
        full_node.set_server(cast(AppleServer, FakeServer()))
        async with full_node.manage():
            ...
            peer: WSAppleConnection = cast(WSAppleConnection, FakePeer())
...
async def run_sync_checkpoint(
    ...
    apple_init(root_path, should_check_keys=False, v1_db=False)
    config = load_config(root_path, "config.yaml")
    ...
    async with full_node.manage():
        peer: WSAppleConnection = FakePeer()  # type: ignore[assignment]
